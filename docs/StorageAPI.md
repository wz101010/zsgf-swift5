...
# **storagePut**
```swift
    open class func storagePut(table: String, id: String, appKey: String, body: AnyCodable, replace: Bool? = nil, completion: @escaping (_ data: BooleanApiResponse?, _ error: Error?) -> Void)
```
...
let id = "id_example" // String | 数据ID
let appKey = "appKey_example" // String | 
let body =  // AnyCodable | json格式
let replace = true // Bool | 是否为全量更新，默认为false (optional) (default to false)

// 更新数据
StorageAPI.storagePut(table: table, id: id, appKey: appKey, body: body, replace: replace) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
